function [state,Energy_losses,labels_Energy,etaMec,etaCyclen,etaToten,Exergy_losses,labels_Ex,etaRotex,etaCyclex,etaCombex,etaTotex,ma,mc,mg,lambda] = mainTurbineGaz(T1,r,etaPiC,kcc,T3,etaPiT,Pe,x,y,z)
...
state{1}.T = T1;
%state{1}.h = janaf('h','N2',T1)*mN2_a + janaf('h','O2',T1)*mO2_a;
%state{1}.s = janaf('s','N2',T1)*mN2_a + janaf('s','O2',T1)*mO2_a;
state{1}.h = 15.1;
state{1}.s = 0.054;
%%%% State 2 %%%%
...
state{2}.T = T2;
state{2}.h = janaf('h','N2',T2)*mN2_a + janaf('h','O2',T2)*mO2_a;
state{2}.s = janaf('s','N2',T2)*mN2_a + janaf('s','O2',T2)*mO2_a;
...
state{3}.h = mCO2_g*janaf('h','CO2',T3) + mH2O_g*janaf('h','H2O',T3)...
    + mO2_g*janaf('h','O2',T3) + mN2_g*janaf('h','N2',T3);
disp(state{3}.h);
state{4}.h = mCO2_g*janaf('h','CO2',T4) + mH2O_g*janaf('h','H2O',T4)...
    + mO2_g*janaf('h','O2',T4) + mN2_g*janaf('h','N2',T4);
disp(state{4}.h);
[ma,mc,mg,lambda] = solverFlow(state,x,y,z,kmec,Pe);
...
state{3}.s = mCO2_g*janaf('s','CO2',T3) + mH2O_g*janaf('s','H2O',T3)...
    + mO2_g*janaf('s','O2',T3) + mN2_g*janaf('s','N2',T3);
state{4}.s = mCO2_g*janaf('s','CO2',T4) + mH2O_g*janaf('s','H2O',T4)...
    + mO2_g*janaf('s','O2',T4) + mN2_g*janaf('s','N2',T4);
...
%%%% Energy analysis %%%%
[Energy_losses,labels_Energy,etaMec,etaCyclen,etaToten] = energyAnalysis4(state,ma,mc,mg,Pe,x,y,z);

%%%% Exergy analysis %%%%
[Exergy_losses,labels_Ex,etaMec,etaRotex,etaCyclex,etaCombex,etaTotex] = exergyAnalysis4(state,ma,mc,mg,x,y,z,Pe);
